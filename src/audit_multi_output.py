...
- For each problem, asks an LLM (default model: gpt-5) whether any test input
...
Usage:
  python src/audit_multi_output.py \
    ...
    --model gpt-5 --output runs/multi_output_audit.json

Environment:
  Requires OPENAI_API_KEY if using the OpenAI API. Use --dry-run or --heuristic-only
...
def call_openai_responses(model: str, prompt: str, timeout: int = 60) -> Dict[str, Any]:
    """Call OpenAI Responses API with a single prompt, expecting JSON output.

    Uses the same API pattern as elsewhere in this repo (no response_format kwarg),
    and parses the text from response.output, then json.loads it.

    Returns a dict parsed from the model's JSON. On failure, raises Exception.
    """
    try:
        # Deferred import so the script can still run in heuristic-only or dry-run modes.
...
def main() -> None:
    ...
    for i, prob in enumerate(selected, 1):
        ...
        if args.dry_run or args.heuristic_only:
            ...
            try:
                llm_json = call_openai_responses(model=args.model, prompt=prompt)
                result_entry.update({
